...
class MyFrame(wx.Frame):
  def __init__(self):
    ...
    # add a menu 
    menu = wx.Menu()
    self.Bind(wx.EVT_CONTEXT_MENU, self.OnMouseLeftDown)
...
#    self.Bind(wx.EVT_MENU, self.OnColor,id=203)
  ...
  def OnColor(self, evt):
    #if(evt.GetId()==201):
    item = self.popmenu.FindItemById(evt.GetId())
#    text = item.GetText()
...
#    self.GetMenuBar().Enable(self.IdCommand, self.changeable)
  ...
  def OnMouseLeftDown(self, event):
    ...
    print(self.p1)
    self.p1=self.ScreenToClient(self.p1)
    print(self.p1)
    dc = wx.ClientDC(self)

    self.PopupMenu(self.popmenu,self.p1)
